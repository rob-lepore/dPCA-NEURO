function dpca_plot(Xfull, W, V, dataname, fig_name, fig_count,plotFunction, varargin)
...
%myFig = figure('Render', "painters", 'Position', [0 0 1800 1000], 'Name', 'Demixed-Principal Component Analysis ');
...
%figure name
%sgtitle("dPCA     " + dataname + '  -   Fig. ' + fig_count)
sgtitle( fig_name + '  -  ' + dataname + '  -   Fig. ' + fig_count)
...
if isempty(options.marginalizationColours)
    if ~isempty(options.explainedVar)
        ...
        options.marginalizationColours = margColours %lines(L);
...
colormap([margColours; redBlue256]) %[options.marginalizationColours; redBlue256])
...
if ~isempty(options.explainedVar)
    axPie = subplot(4,4,5);
    %axes('position', [0.205 0.47 0.1 0.1]) %[left bottom width height]
    ...
    h = pie(d, ones(size(d)), margNamesPerc);
%    text_handles = h(2:2:end); %text handles
%    text_handles(2).Position=text_handles(2).Position + [1 0 0];
    caxis([1 length(options.marginalizationColours)+ 256]);
end
...
if ~isempty(options.explainedVar)
    ...
    % pie chart component 
    pos = get(axPie, 'Position');
    set(axPie, 'Position', [pos(1) top+0.02 pos(3) pos(4)])
    pos = get(axPie, 'Position');
    top = pos(2)+pos(4);

    pos = get(axCum, 'Position');
    set(axCum, 'Position', [pos(1) top+0.03 pos(3) pos(4)])
    pos = get(axCum, 'Position');
    top = pos(2)+pos(4);
