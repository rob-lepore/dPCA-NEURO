function dpca_perMarginalization(Xfull, dataname, fig_count, plotFunction, varargin)
...
for m=1:length(Xmargs)
    ...
    if size(Xmargs{m},1)<size(Xmargs{m},2)
        ...
        % [U,S] = eig(XX) returns diagonal matrix S of eigenvalues and
        % matrix U whose columns are the corresponding right eigenvectors,
        % so that XX*U = U*S.
        [U,S] = eig(XX);
...
% Figure = PCA for each marginalization -----------------------------------
myFig = figure('Render', "painters", 'Position', [0 0 1800 1000], 'Name', 'Demixed-Principal Component Analysis');
% figure name
sgtitle("PCA for each marginalization -  " + dataname + '  -   Fig. ' + fig_count)
